 ...
 * @link https://github.com/FSX/php-utf8
 */
...
class UTF8 {
        ...
        for ($i = 0; $i < $len; $i++) {
            ...
            if ($mState == 0) {
                ...
                if (0 == (0x80 & ($in))) {
                    ...
                    Common::halt('A System Error Was Encountered', "UTF8::to_unicode(): Illegal sequence identifier in UTF-8 at byte {$i}", 'sys_error');
                }
            } else {
                ...
                if (0x80 == (0xC0 & ($in))) {
                    ...
                    if (0 == --$mState) {
                        ...
                        if (((2 == $mBytes) && ($mUcs4 < 0x0080)) || ((3 == $mBytes) && ($mUcs4 < 0x0800)) ||
                                ...
                                ($mUcs4 > 0x10FFFF))
                        {
                            Common::halt('A System Error Was Encountered', "UTF8::to_unicode(): Illegal sequence or codepoint in UTF-8 at byte {$i}", 'sys_error');
                        }
                    ...
                    Common::halt('A System Error Was Encountered', "UTF8::to_unicode(): Incomplete multi-octet sequence in UTF-8 at byte {$i}", 'sys_error');
                }
            }
        }
    ...
    private static function from_unicode($arr) {
        ...
        foreach(array_keys($arr) as $k) {
            if(($arr[$k] >= 0) && ($arr[$k] <= 0x007f)) { // ASCII range (including control chars)
                ...
                // Found a surrogate
                Common::halt('A System Error Was Encountered', "UTF8::from_unicode(): Illegal surrogate at index: {$k}, value: {$arr[$k]}", 'sys_error');
                ...
                // Out of range
                Common::halt('A System Error Was Encountered', "UTF8::from_unicode(): Codepoint out of Unicode range at index: {$k}, value: {$arr[$k]}", 'sys_error');
            }
        }
    ...
    public static function wordwrap($str, $width = 75, $break = "\n", $cut = FALSE, $charset = 'utf-8') {
        ...
        if ($break_width === NULL) {
            Common::halt('A System Error Was Encountered', 'UTF8::wordwrap(): Break string cannot be empty', 'sys_error');
        }
        
        if ($width === 0 && $cut) {
            Common::halt('A System Error Was Encountered', 'UTF8::wordwrap(): Cannot force cut when width is zero', 'sys_error');
        }
    ...
    public static function str_pad($input, $length, $pad_str=' ', $type = STR_PAD_RIGHT) {
        ...
        Common::halt('A System Error Was Encountered', "UTF8::str_pad(): Unknown padding type ({$type})", 'sys_error');
    }
    ...
    public static function ord($chr) {
        ...
        if ( ! isset($chr[1])) {
            Common::halt('A System Error Was Encountered', 'UTF8::ord(): Short sequence - at least 2 bytes expected, only 1 seen', 'sys_error');
        }
        ...
        if ( ! isset($chr[2])) {
            Common::halt('A System Error Was Encountered', 'UTF8::ord(): Short sequence - at least 3 bytes expected, only 2 seen', 'sys_error');
        }
        ...
        if ( ! isset($chr[3])) {
            Common::halt('A System Error Was Encountered', 'UTF8::ord(): Short sequence - at least 4 bytes expected, only 3 seen', 'sys_error');
        }
        ...
        if ( ! isset($chr[4])) {
            Common::halt('A System Error Was Encountered', 'UTF8::ord(): Short sequence - at least 5 bytes expected, only 4 seen', 'sys_error');
        }
        ...
        if ( ! isset($chr[5])) {
            Common::halt('A System Error Was Encountered', 'UTF8::ord(): Short sequence - at least 6 bytes expected, only 5 seen', 'sys_error');
        }
        ...
        if ($ord0 >= 254 && $ord0 <= 255) {
            Common::halt('A System Error Was Encountered', "UTF8::ord(): Invalid UTF-8 with surrogate ordinal {$ord0}", 'sys_error');
        }
    }
