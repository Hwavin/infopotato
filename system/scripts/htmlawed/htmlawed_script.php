 ...
 * Main function
 *
 * The output of htmLawed is a text string containing the processed input. 
 *
 * @access	public
 * @param	string	the input text to be processed
 * @param	string	configurable parameters 
 * @param	mixed 	specifying element-specific attribute and attribute value restrictions
 * @return	string
 */
...
/**
 * Checking attribute values against $spec
 */
...
function hl_prot($p, $c = NULL) {
	...
	if (isset($c['*']) or !strcspn($p, '#?;') or (substr($p, 0, 4) == 'deni')) {
		return "{$b}{$p}{$a}";
...
/**
 * Checking syntax of a regular expression
 */
function hl_regex($p) {
	// ?regex
	if (empty($p)) {
