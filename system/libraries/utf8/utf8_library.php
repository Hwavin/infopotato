...
class UTF8_Library {
	 ...
	 * @var boolean
	 */
	private $_can_ignore_invalid = NULL;
	 ...
	 * @var array
	 */
	private $_mb_lower_to_upper_fix = array(
	 ...
	 * @var array
	 */
	private $_mb_upper_to_lower_fix = array(
	 ...
	 * @var boolean
	 */
	private $_mbstring_available = NULL;
	...
	public function ascii($string) {
		...
		$string = strtr($string, $this->_utf8_to_ascii);
	 ...
	 * @return void
	 */
	private function _check_mbstring() {
		$this->_mbstring_available = extension_loaded('mbstring');
	}
	 ...
	 */
	public function chr($unicode_code_point) {
	...
	public function clean($value) {
		if ( ! is_array($value)) {
			if ($this->_can_ignore_invalid === NULL) {
				$this->_can_ignore_invalid = strtolower(ICONV_IMPL) != 'unknown';	
			}
			return iconv('UTF-8', 'UTF-8' . ($this->_can_ignore_invalid ? '//IGNORE' : ''), (string) $value);
		}
	...
	public function cmp($str1, $str2) {
		$ascii_str1 = strtr($str1, $this->_utf8_to_ascii);
		$ascii_str2 = strtr($str2, $this->_utf8_to_ascii);
	 ...
	 */
	private function _convert_offset_2_bytes($string, $offset) {
		if ($offset == 0) {
			return 0;
		}
	 ...
	 */
	public function explode($string, $delimiter = NULL) {
	 ...
	 * @return integer  < 0 if $str1 < $str2, 0 if they are equal, > 0 if $str1 > $str2
	 */
	public function icmp($str1, $str2) {
		$str1 = $this->lower($str1);
		$str2 = $this->lower($str2);
		
		return $this->cmp($str1, $str2);
	}
	 ...
	 * @return integer  `< 0` if `$str1 < $str2`, `0` if they are equal, `> 0` if `$str1 > $str2`
	 */
	public function inatcmp($str1, $str2) {
		$str1 = $this->lower($str1);
		$str2 = $this->lower($str2);
		
		return $this->natcmp($str1, $str2);
	}
	 ...
	 */
	public function ipos($haystack, $needle, $offset = 0) {
		// We get better performance falling back for ASCII strings
		if ( ! $this->_detect($haystack)) {
			return stripos($haystack, $needle, $offset);
		}
		
		if ($this->_mbstring_available === NULL) {
			$this->_check_mbstring();
		}
		
		if ($this->_mbstring_available && function_exists('mb_stripos')) {
		...
		return $this->pos($haystack, $needle, $offset);
	}
	 ...
	 */
	public function irpos($haystack, $needle, $offset = 0) {
		// We get better performance falling back for ASCII strings
		if ( ! $this->_detect($haystack)) {
		...
		if ($this->_mbstring_available && function_exists('mb_strripos')) {
		...
		return $this->rpos($haystack, $needle, $offset);
	}
	 ...
	 */
	public function istr($haystack, $needle, $before_needle = FALSE) {
		// We get better performance falling back for ASCII strings
		if ($before_needle == FALSE && ! $this->_detect($haystack)) {
		...
		if ($this->_mbstring_available && function_exists('mb_stristr')) {
			return mb_stristr($haystack, $needle, $before_needle, 'UTF-8');
		}
		
		$lower_haystack = $this->lower($haystack);
		$lower_needle = $this->lower($needle);
	...
	public function lower($string) {
		...
		if ($this->_mbstring_available) {
			...
			return strtr($string, $this->_mb_upper_to_lower_fix);
		}
		
		return strtr($string, $this->_upper_to_lower);
	}
	 ...
	 */
	public function ltrim($string, $charlist = NULL) {
	 ...
	 * @return integer  `< 0` if `$str1 < $str2`, `0` if they are equal, `> 0` if `$str1 > $str2`
	 */
	public function natcmp($str1, $str2) {
		$ascii_str1 = strtr($str1, $this->_utf8_to_ascii);
		$ascii_str2 = strtr($str2, $this->_utf8_to_ascii);
	...
	public function ord($character) {
		...
		if ($invalid) {
			echo 'The UTF-8 character specified is invalid'
		}
	 ...
	 */
	public function pad($string, $pad_length, $pad_string = ' ', $pad_type = 'right') {
		...
		// We get better performance falling back for ASCII strings
		if ( ! $this->_detect($string) && ! $this->_detect($pad_string)) {
		...
		$string_length = $this->len($string);
		$pad_string_length = $this->len($pad_string);
		...
		while ($padded < $pad_to_length) {
			...
			if ($pad_to_length - $padded < $pad_string_length) {
				$pad_string = $this->sub($pad_string, 0, $pad_to_length - $padded);
			}
	 ...
	 * @return mixed  The integer character position of the first occurence of the needle or `FALSE` if no match
	 */
	public function pos($haystack, $needle, $offset = 0) {
		if ($this->_mbstring_available === NULL) {
			$this->_check_mbstring();
		}
		...
		$offset = $this->_convert_offset_2_bytes($haystack, $offset);
	 ...
	 * @return mixed  The integer character position of the last occurence of the needle or `FALSE` if no match
	 */
	public function rpos($haystack, $needle, $offset = 0) {
		// We get better performance falling back for ASCII strings
		if ( ! $this->_detect($haystack)) {
	 ...
	 * @return mixed  The specified part of the haystack, or `FALSE` if the needle was not found
	 */
	public function str($haystack, $needle, $before_needle = FALSE) {
		if ($this->_mbstring_available === NULL) {
			$this->_check_mbstring();
		}
		
		if ($this->_mbstring_available && function_exists('mb_strstr')) {
	...
	public function sub($string, $start, $length = NULL) {
		...
		$start = $this->_convert_offset_2_bytes($string, $start);
		...
		$length = $this->_convert_offset_2_bytes($string, $length);
	...
	public function ucfirst($string) {
		return $this->upper($this->sub($string, 0, 1)) . $this->sub($string, 1);
	}
	 ...
	 */
	private function _ucwords_callback($match) {
	...
	public function upper($string) {
		...
		if ($this->_mbstring_available) {
			...
			return strtr($string, $this->_mb_lower_to_upper_fix);
		}
		
		return strtr($string, $this->_lower_to_upper);
	}
	...
	public function wordwrap($string, $width = 75, $break = "\n", $cut = FALSE) {
		...
		foreach ($words as $word) {
			$word_len = $this->len($word);
			
			// Shorten up words that are too long
			while ($cut && $word_len > $width) {
				$output .= $break;
				$output .= $this->sub($word, 0, $width);
				$line_len = $width;
				$word = $this->sub($word, $width);
				$word_len = $this->len($word);
			}
...
/* End of file: ./system/libraries/utf8/utf8_library.php */