<?php
 class Dump { public static $xml_CDATA; public static $xml_SDATA; public static $xml_DDATA; public static $xml_count = 0; public static $xml_attrib; public static $xml_name; public static $arr_type = array('array', 'object', 'resource', 'boolean', 'NULL'); public static $collapsed = FALSE; public static $arr_history = array(); public static function show($var, $force_type = '', $collapsed = FALSE) { if ( ! defined('DUMP_INIT')) { define('DUMP_INIT', TRUE); self::init_JS_and_CSS(); } self::$collapsed = $collapsed; switch (strtolower($force_type)) { case 'array': self::var_is_array($var); break; case 'object': self::var_is_object($var); break; case 'xml': self::var_is_xml_resource($var); break; default: self::check_type($var); } } public static function make_table_header($type, $header, $colspan = 2) { $str_i = (self::$collapsed) ? "style=\"font-style:italic\" " : ''; echo "<table cellspacing=\"2\" cellpadding=\"3\" class=\"dump_".$type."\">
				<tr>
